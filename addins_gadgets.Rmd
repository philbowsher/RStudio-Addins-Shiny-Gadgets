...
## How do I share a package (since an addin is just a package) I’ve created within my organization?

- https://support.rstudio.com/hc/en-us/articles/115000239587-Sharing-Internal-R-Packages

 - Note: Pre-req is you have a package ... there are a few resources online that show how to use RStudio to create an R package
 - https://support.rstudio.com/hc/en-us/articles/200486488-Developing-Packages-with-RStudio
 
 - Option A: Share File
 - You will want to build the package into a source / tar file. This is done in the IDE with "Build Source Package" within the Build tab.
 - You can then put that tar file in a shared network location. Users can download the tar file and then install it with:
 
```{r, eval=FALSE}
 install.packages("path/to/tar/file", source = TRUE)
```

- Option B: Use Git
- You can put the whole R package (the R folder, namespace file, description, etc) into a git repo (either an internal repo or github).
Then users can get the package using devtools

```{r, eval=FALSE}
 devtools::install_git(repo_url)
```

 - Option C: Use Local Repo
 - These instructions: https://rstudio.github.io/packrat/custom-repos.html will provide a framework for setting up a local repo and adding packages to it. In particular:
 - 1. Update the package’s DESCRIPTION file to have a home repo and build it (A.1).
 - 2. Move the tar file into the correct location for your local repo.
 - 3. Update the repo’s PACKAGE files using tools::write_PACKAGES

Then users can install the package with

```{r, eval=FALSE}
install.packages('package_name', source = TRUE)
```

*repos = NULL, # Tells R not to try to get the package from CRAN

- Option D: System Library
- If you are using RSP, you can make your addin automatically available to users by installing the package in the system library. To do so, just use one of the methods above (A-C) and install the package into the system location:

```{r, eval=FALSE}
install.packages('my_package', source=TRUE, lib.loc = "/path/to/system/lib")
```
...
- https://datasciencespecialization.github.io/Developing_Data_Products/Shiny_Gadgets/Shiny_Gadgets.html#1
...
### Interesting Addins
 ...
 - https://stevenmortimer.com/2016/12/10/creating-an-rstudio-addin/
 - http://deanattali.com/blog/plot-colour-helper/
